...
// <echo message="..."/>
{
  ...
  handler: match => {
    ...
    const processed = rawMessage.replace(/\{([^}]+)\}/g, (_, varName) => `\${slykeVariables["${varName}"].value}`);
    return `
...
// message "..."
{
  ...
  handler: match => {
    const rawMessage = match[1];
    const processed = rawMessage.replace(/\{([^}]+)\}/g, (_, v) => `\${slykeVariables["${v}"].value}`);
    return `
      compiledElements.push(() =>
...
// popUp "..."
{
  ...
  handler: match => {
    ...
    const messageWithSignals = rawMessage.replace(/\{([^}]+)\}/g, (_, v) => `\${slykeVariables["${v}"].value}`);
    if (condition) {
      return `
        compiledElements.push(() =>
